...
export let NavigationMenu = class NavigationMenu {

  static inject() {
    return [AppRouter, CompositionEngine, EventAggregator, Container];
  }

  constructor(appRouter, compositionEngine, eventAggregator, container) {
    this.appRouter = appRouter;
    this.compositionEngine = compositionEngine;
    this.eventAggregator = eventAggregator;
    this.container = container;
    this.menu = [];
    this._configureMenuPromise = new Promise(resolve => {
      this._resolveMenuPromise = resolve;
    });
    this.setupEvents();
  }
  ...
  setupEvents() {
    ...
    this.subscriber = this.eventAggregator.subscribe('router:navigation:complete', response => {
      this.updateMenu(response.instruction, 0);
    });
  }

  configureMenu() {
    return this.loadChildRouters(this.appRouter, this.container.root.viewModel).then(i => {
      this.menu = i;
  ...
  loadChildRouters(router, viewModel) {
    ...
    return promise.then(() => {
      ...
      let routes = router.routes.filter(i => {
        if (i.route === "" && router.routes.some(r => r.moduleId === i.moduleId && r.name === i.name)) return false;
      ...
      return Promise.all(loads).then(() => router.navigation);
    });
  }
  ...
  loadChildRouter(router, navModel) {
    ...
    return this.compositionEngine.ensureViewModel(instruction).then(component => {
